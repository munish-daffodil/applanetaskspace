...
exports.createTask = function (req, callback) {
    ...
    DataBaseManager.insert(Constants.Task.COLLECTION, newTask, function (err, task) {
        if (err) {
            callback(err);
            return;
        }
...
asWatch = function (watch, watches) {
    if (!watch || !watches) {
        return false;
    }
    var watchCount = watches.length;
...
exports.removeComment = function (req, callback) {
    ...
    DataBaseManager.update(Constants.Task.COLLECTION, query, update, function (err, data) {
        if (err) {
            callback(err);
            return;
        }
...
validatePriority = function (priority) {
    ...
    } else if (Constants.Task.Priority.LOW == priority) {
        return Constants.Task.Priority.LOW;
    } else {
        return Constants.Task.Priority.LOW;
    }
