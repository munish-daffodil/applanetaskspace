...
exports.createTask = function (req, callback) {
    var user = req._user;
    var shortDesc = trimValue(req.param(Constants.Task.SHORT_DESCRIPTION));
    if (!shortDesc) {
        callback(new AppError(Constants.ErrorCode.FIELDS_BLANK, [Constants.Task.SHORT_DESCRIPTION]));
        return;
    }
    ...
    var newTask= {};
    newTask[Constants.Task.SHORT_DESCRIPTION] = shortDesc;
...
exports.updateTask = function (req, callback) {
    ...
    DataBaseManager.findOne(Constants.Task.COLLECTION, query, function (err, taskInfo) {
        ...
        var shortDesc = trimValue(req.param(Constants.Task.SHORT_DESCRIPTION));
        if (shortDesc) {
            $set[Constants.Task.SHORT_DESCRIPTION] = shortDesc;
        }
...
validateStatus = function (status) {
    if (!status) {
        ...
        //TODO invalid status, need to handle
    }
}
